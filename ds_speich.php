...
try {
    ...
    // --- Stammdaten (Nur für berechtigte User) ---
    if ($can_edit_master_data) {
        ...
        $data_to_save['hinw'] = trim($_POST['hinw'] ?? $old_record['hinw']); // Nimmt langen Hinweis entgegen
        ...
        // Wenn nicht berechtigt, alten Stand beibehalten
        $data_to_save = array_merge($data_to_save, [
            'sachnr' => $old_record['sachnr'], 'kurz' => $old_record['kurz'],
            'aez' => $old_record['aez'], 'dokart' => $old_record['dokart'],
            'teildok' => $old_record['teildok'], 'hinw' => $old_record['hinw']
        ]);
        for ($i = 1; $i <= 7; $i++) {
    ...
    // --- DATUM KORREKT ÜBERNEHMEN ---
    // Nimmt das Datum aus dem Formular. Wenn es leer ist, wird das alte Datum als Fallback verwendet.
    $data_to_save['dat'] = trim($_POST['dat'] ?? $old_record['dat']);

    // --- Record-Status (Nur für spezielle Admins) ---
    if ($is_special_admin) {
    ...
    // --- Checkbox-Status für Abteilungen verarbeiten ---
    $submitted_dept_checkboxes = $_POST['department_status_checkbox'] ?? [];
    for ($i = 1; $i <= 7; $i++) {
        // Logik für Abteilungsstatus bleibt unverändert...
    ...
    // --- LOGGING-LOGIK (INKLUSIVE DATUM) ---
    $changes = [];
    foreach ($data_to_save as $key => $new_value) {
        // Prüfen, ob sich der Wert vom alten Datensatz unterscheidet.
        // Die Prüfung 'key !== 'dat'' wurde entfernt, um Datumsänderungen zu loggen.
        if (array_key_exists($key, $old_record) && $old_record[$key] != $new_value) {
            $changes[$key] = [
                'alt' => $old_record[$key],
                'neu' => $new_value
            ];
    ...
    // --- Datenbank-Update ausführen, falls es Änderungen gab ---
    if (!empty($changes)) {
        // SQL-Query mit allen Feldern vorbereiten
        ...
        // Log-Eintrag nur erstellen, wenn die DB-Aktion erfolgreich war
        ...
        $_SESSION['success_message'] = "Keine Änderungen für Datensatz (ID: " . htmlspecialchars($id) . ") festgestellt.";
    }
